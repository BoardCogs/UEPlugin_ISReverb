...
#include "ISReverbAPI.generated.h"


/** Mathematic camera model for ISReverb/undistortion.
 *
 * Camera matrix =
 *  | F.X  0  C.x |
 *  |  0  F.Y C.Y |
...
struct FISReverbCameraModel
{
	GENERATED_USTRUCT_BODY()
	FISReverbCameraModel()
	{
		K1 = K2 = K3 = P1 = P2 = 0.f;
		F = FVector2D(1.f, 1.f);
	...
	/** Radial parameter #1. */
	UPROPERTY(Interp, EditAnywhere, BlueprintReadWrite, Category = "ISReverb|Camera Model")
	float K1;
	...
	/** Compare two ISReverb models and return whether they are equal. */
	bool operator == (const FISReverbCameraModel& Other) const
	{
		return (
			K1 == Other.K1 &&
			K2 == Other.K2 &&
	...
	/** Compare two ISReverb models and return whether they are different. */
	bool operator != (const FISReverbCameraModel& Other) const
	{
		return !(*this == Other);
	}
