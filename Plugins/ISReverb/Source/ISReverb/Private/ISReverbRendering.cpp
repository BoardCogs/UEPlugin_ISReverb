...
#define LOCTEXT_NAMESPACE "ISReverbPlugin"



/**
 * Internal intermediary structure derived from FISReverbCameraModel by the game thread
...
struct FCompiledCameraModel
{
	...
	FISReverbCameraModel OriginalCameraModel;

	/** Camera matrices of the ISReverb for the undistorted and distorted render.
...
class FISReverbUVGenerationShader : public FGlobalShader
{
	DECLARE_INLINE_TYPE_LAYOUT(FISReverbUVGenerationShader, NonVirtual);
public:
	...
	FISReverbUVGenerationShader() {}

	FISReverbUVGenerationShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
...
class FISReverbUVGenerationVS : public FISReverbUVGenerationShader
{
	DECLARE_SHADER_TYPE(FISReverbUVGenerationVS, Global);
public:

	/** Default constructor. */
	FISReverbUVGenerationVS() {}

	/** Initialization constructor. */
	FISReverbUVGenerationVS(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
		: FISReverbUVGenerationShader(Initializer)
	{
	}
};


class FISReverbUVGenerationPS : public FISReverbUVGenerationShader
{
	DECLARE_SHADER_TYPE(FISReverbUVGenerationPS, Global);
public:

	/** Default constructor. */
	FISReverbUVGenerationPS() {}

	/** Initialization constructor. */
	FISReverbUVGenerationPS(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
		: FISReverbUVGenerationShader(Initializer)
	{ }
};


IMPLEMENT_SHADER_TYPE(, FISReverbUVGenerationVS, TEXT("/Plugin/ISReverb/Private/SoundShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FISReverbUVGenerationPS, TEXT("/Plugin/ISReverb/Private/SoundShader.usf"), TEXT("MainPS"), SF_Pixel)
	...
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	SCOPED_DRAW_EVENTF(RHICmdList, SceneCapture, TEXT("ISReverbDisplacementGeneration %s"), TextureRenderTargetName);
	...
	RHICmdList.BeginRenderPass(RPInfo, TEXT("DrawUVDisplacement"));
	{
		...
		TShaderMapRef< FISReverbUVGenerationVS > VertexShader(GlobalShaderMap);
		TShaderMapRef< FISReverbUVGenerationPS > PixelShader(GlobalShaderMap);
...
FVector2D FISReverbCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FISReverbCameraModel::GetUndistortOverscanFactor(
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	// If the ISReverb model is identity, then early return 1.
	if (*this == FISReverbCameraModel())
	{
		return 1.0f;
	}
	...
	// in case of a tangential distorted barrel ISReverb.
...
void FISReverbCameraModel::DrawUVDisplacementToRenderTarget(
	UWorld* World,
	...
	float OutputAdd) const
{
	...
	{
		FMessageLog("Blueprint").Warning(LOCTEXT("ISReverbCameraModel_OutputTargetRequired", "DrawUVDisplacementToRenderTarget: Output render target is required."));
		return;
	}
	...
	{
		FMessageLog("Blueprint").Warning(LOCTEXT("ISReverbCameraModel_SM5Unavailable", "DrawUVDisplacementToRenderTarget: Requires RHIFeatureLevel::SM5 which is unavailable."));
		return;
	}
