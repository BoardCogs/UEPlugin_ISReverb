#include "TmpIS_Source.h"



// Sets default values
ATmpIS_Source::ATmpIS_Source()
{
...
void ATmpIS_Source::GenerateISs()
{
	// Getting all listeners
	TArray<AActor*> listeners; 
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), ATmpIS_Listener::StaticClass(), listeners);
	...
	for (AActor* actor : listeners)
	{
		...
		ATmpIS_Listener* listener = Cast<ATmpIS_Listener>(actor);
...
void ATmpIS_Source::GenerateISsLinear(ATmpIS_Listener* listener, FVector3f position)
{
	// Generates ISTree and adds it to the array
	TmpIS_Tree tree = TmpIS_Tree(order, position, listener->GetRooms(), WrongSideOfReflector, BeamTracing, BeamClipping, debugBeamTracing);
...
TFuture<TmpIS_Tree> ATmpIS_Source::CreateISTreeTask(ATmpIS_Listener* listener, FVector3f position)
{
	TSharedRef<TPromise<TmpIS_Tree>> Promise = MakeShared<TPromise<TmpIS_Tree>>();
	TFuture<TmpIS_Tree> Future = Promise->GetFuture();
...
void ATmpIS_Source::GenerateAllReflectionPaths()
{
    if (trees.IsEmpty())
        return;
    
	for (TPair<ATmpIS_Listener*, TmpIS_Tree>& pair : trees)
	{
		GenerateRP(pair.Key);
	}
}
...
void ATmpIS_Source::GenerateRP(ATmpIS_Listener* listener)
{
...
void ATmpIS_Source::GenerateRPLinear(ATmpIS_Listener* listener)
{
	FDateTime StartTime = FDateTime::UtcNow();
	
	FVector3f listenerPos = FVector3f( listener->GetTransform().TransformPosition(FVector3d(0,0,0)) );

	TArray<TmpIS*> nodes = trees[listener].Nodes();

	int validPaths = 0;

	for (TmpIS* node : nodes)
	{
		FHitResult hit;
		...
		if (node->Valid)
		{
			...
			while (currentIndex != -1)
			{
				...
				{
					ATmpIS_ReflectorSurface* hitSurface = Cast<ATmpIS_ReflectorSurface>( hit.GetActor() );
...
void ATmpIS_Source::GenerateRPMT(ATmpIS_Listener* listener)
{
...
int ATmpIS_Source::LinePlaneIntersection(FVector3f* intersection, FVector3f linePoint, FVector3f lineVec, FVector3f planeNormal, FVector3f planePoint, double epsilon)
{
...
bool ATmpIS_Source::RoomsInCommon(TArray<ATmpIS_Room*> a, TArray<ATmpIS_Room*> b)
{
	for (ATmpIS_Room* room : a)
	{
		if (b.Contains(room))
			return true;
	}
...
void ATmpIS_Source::UpdateCurrentRoom()
{
...
void ATmpIS_Source::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	// Saving a backup of current rooms
	TArray<ATmpIS_Room*> RoomsBackup = _rooms;
...
void ATmpIS_Source::DrawDebug()
{
	...
	if (drawImageSources)
	{
		...
		if (trees.Num() > 0)
		{
			...
			TArray<ATmpIS_Listener*> listeners; 
			trees.GetKeys(listeners);
    	
			for (TmpIS* node : trees[listeners[0]].Nodes())
			{
				if (node->Valid == true)
	...
	if (checkNode != -1)
	{
		if (trees.Num() > 0)
		{
			...
			TArray<ATmpIS_Listener*> listeners; 
			trees.GetKeys(listeners);
			TArray<TmpIS*> nodes = trees[listeners[0]].Nodes();
			
			if (checkNode >= 0 && checkNode < nodes.Num())
			{
				TmpIS* node = nodes[checkNode];
				FColor color;
	...
	if (checkNode != -1)
	{
		if (trees.Num() > 0)
		{
			...
			if (checkNode >= 0 && checkNode < nodes.Num() && nodes[checkNode]->Parent != -1)
			{
				...
				for (TmpIS_ReflectorEdge edge : node->BeamPoints.Edges())
				{
