 ...
 */
class UEPLUGIN_ISREVERB_API TmpIS_Tree
{
public:
	// CONSTRUCTOR
    ...
    TmpIS_Tree(int r, FVector3f sourcePos, TArray<ATmpIS_Room*> rooms, bool wrongSideOfReflector, bool beamTracing, bool beamClipping, bool debugBeamTracing);
...
private:
    ...
    TArray<TmpIS> _nodes = TArray<TmpIS>();
	...
	TArray<ATmpIS_ReflectorSurface*> _surfaces;

	// METHODS
    // All reflectors in the scene
    TArray<ATmpIS_ReflectorSurface*> Surfaces();

    // This function checks all conditions for creating a new Image Source, then creates it if all are respected
    bool CreateIS(int order, int parent, ATmpIS_ReflectorSurface* surface, TArray<FVector3f> projectionPlanesNormals, FCriticalSection& nodeLock, FCriticalSection& noDoubleLock, FCriticalSection& wrongSideLock, FCriticalSection& beamLock, FCriticalSection& realISsLock);
    ...
    TArray<FVector3f> CreateProjectionPlanes(FVector3f position, TmpIS_BeamProjection BeamProjection);
